					...
					<div class="card-body">
						...
						<div class="tab-content py-3">
							<div class="tab-pane fade show active" id="primaryhome" role="tabpanel">
								...
								<form class="row g-2" method="POST" action="{{route('update_module')}}" enctype="multipart/form-data">
									{{csrf_field()}}
									<input type="hidden"  name="id"  value="{{$mobile_m->id}}" >

									<div class="col-md-3">
									<label for="inputFirstName" class="form-label">Select Mobile Brand</label>
										<select class="form-select mb-3" aria-label="Default select example" name="select_mobile_brand" id="mobilebrand">
										<option value="">Select Category</option>
		                                             
										
                                        @foreach ($mobile_type as $mobiles_series)
                                                    <option value="{{$mobiles_series->id }}"
														@if ($mobile_m->select_mobile_brand_id==$mobiles_series->id)selected @endif>
									...
									<label for="inputFirstName" class="form-label">Select Mobile Series</label>
										<select class="form-select mb-3" aria-label="Default select example" name="select_mobile_series" id="mobileseries">
											
											<option value="">Select Mobile Series</option>
											@foreach($mobile_sub as $mobile_module)
											<option value="{{$mobile_module->id}}"@if($mobile_m->select_mobile_series_id==$mobile_module->id)selected @endif>{{$mobile_module->add_mobile_series}}</option>
											@endforeach
									...
									<div class="col-md-3">
										...
										<input type="text" class="form-control" id="inputFirstName" placeholder="Add Mobile Model" name="add_mobile_module" value="{{$mobile_m->add_mobile_module}}">
									</div>
									...
									<div class="col-md-3">
										...
										<label for="inputFirstName" class="form-label">(Need 50 X 50 pixels in png format)</label><img height="50px" width="50px" src="{{asset('public')."/".$mobile_m->mobile_model_image}}" alt="">
									</div>
									

									<div class="col-md-2">
										<label for="inputFirstName" class="form-label">Select RAM</label>
										<select class="form-select mb-3" aria-label="Default select example" name="ram" value="{{$mobile_m->ram}}"
											id="ram">
									...
									<div class="col-md-2">
										...
										<select class="form-select mb-3" aria-label="Default select example"  name="storage" value="{{$mobile_m->storage}}"
											id="storage">
									...
									<div class="col-md-2">
										...
										<input type="text" class="form-control" id="price"
											placeholder="Add Mobile Prices" name="price" >
											...
											<div class="col-md-6 " style="float-left">
												<table class="items_table table table-bordered width80" id="table">
													...
													<tbody class="add_more">
														@foreach ($module_list as $module_lists)	
														<tr><td><input type="hidden" name="existing_id[]" value="{{$module_lists->id}}"><input type="text" name="ram[]" required="" style="border:none; width: 100%;" value="{{$module_lists->ram}}"></td><td><input type="text" name="storage[]" required="" style="border:none; width: 100%;" value="{{$module_lists->storage}}"></td><td><input type="text" name="price[]" required="" style="border:none; width: 100%;" value="{{$module_lists->price}}"></td></td><td><a href="{{route('delete_added_table',$module_lists->id)}}"><button type="button" class="btn1 btn-outline-danger delete-row"><i class="bx bx-trash me-0"></i></button></a></td></tr>
														@endforeach
									...
									<div class="table-responsive">
										<table id="example111" class="table table-striped table-bordered">
											...
											<tbody>
												...
												@foreach ($mobile_modu as $mobiles)
													<tr>
														<td>{{ $loop->index + 1 }}</td>
														...
														<td>
															<button type="button" class="btn btn-primary px-3 viewinfo"
																...
																<a href="{{ route('edit_module', $mobiles->mobile_modules_id) }}">
																	<button type="button" class="btn1 btn-outline-success"><i
																			class='bx bx-edit-alt me-0'></i></button></a>
															<a
																href="{{ route('destroy_module', $mobiles->mobile_modules_id) }}">
