					...
					<div class="col-md-8 mx-auto">
						<div class="card">
							<div class="card-body">
								...
								<hr>
								<form class="row g-3" method="POST" action="{{route('update_maintenance')}}" enctype="multipart/form-data" >
                                {{csrf_field()}}
                                <input type="hidden" name="id" value="{{$mainten->id}}">
									<div class="col-md-5">
										<label for="inputFirstName" class="form-label">Reason For Maintenance</label>
										<input class="form-control"  placeholder="Reason For Maintenance" name="reason_maintenance" value="{{$mainten->reason_maintenance}}"> 
									</div>


									<div class="col-md-5">
										<label for="inputFirstName" class="form-label">Select Status</label>
										<select class="form-select mb-3" aria-label="Default select example" name="status" value="{{$mainten->status}}">
											<option selected>Select</option>
											<option value="Activate" @if ($mainten->status == 'Activate') selected @endif>Activate</option>
											<option value="Deactivate"  @if ($mainten->status == 'Deactivate') selected @endif>Deactivate</option>
										</select>
									</div>
